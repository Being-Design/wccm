...
//			echo '<a href="' . get_permalink( $post_id ) . '" title="' . esc_attr( $post_title ) . '" class="course-cover-image">' . $img . '</a>';
        ...
        <section class="entry">
			...
			<?php
				...
				// echo groups_get_total_member_count(get_post_meta($post_id, 'bp_course_group', true)); 

				$group_id = get_post_meta($post_id, 'bp_course_group', true);
				$student_count = get_group_student_count( $group_id ); 
			?> 
			<span class="member-count"><?php echo ( $student_count > 1 ) ? $student_count . ' Members' : $student_count . ' Member'; ?></span>
